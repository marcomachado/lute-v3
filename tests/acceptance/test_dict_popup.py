...
def test_dictionary_popup_reuses_window(luteclient):
    ...
    assert len(context.pages) == initial_page_count + 1, "new page was opened"
    ...
    assert (
        len(context.pages) == initial_page_count + 1
    ), "existing popup window was reused"
...
def test_dictionary_popup_closed_on_unload(luteclient):
    ...
    assert len(context.pages) == initial_page_count, "popup window closed on unload"
